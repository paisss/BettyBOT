...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        console.log(color(figlet.textSync('Betty BOT', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, 'Halo aku Betty, Ketik #help Untuk Melihat List Command Ku...')
            if(mtcState === false){
                ...
                if(groups.length > groupLimit){
                    ...
                    if(chat.groupMetadata.participants.length < memberLimit){
                        ...
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Halo aku Betty, Ketik #help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'Betty sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
